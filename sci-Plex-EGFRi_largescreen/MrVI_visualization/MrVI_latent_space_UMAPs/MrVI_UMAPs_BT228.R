...
cds_BT228 <- cds[,colData(cds)$cell_line == "BT228"]
temp_col_data_rownames <- rownames(colData(cds_BT228) %>% as.data.frame())

# ================================================================================
# UMAPs of U latent spaces
# ================================================================================

MrVI_u_space <- read_csv("bt228_mrvi_latent_u_umap_02242024.csv", 
                         col_names = TRUE, 
                         col_select = -1) %>%
  as.data.frame() %>%
...
reducedDims(cds_BT228)[["UMAP_U"]] <- MrVI_u_space

plot_cells_U(cds_BT228, reduction_method = "UMAP_U",
...
ggsave("BT228_Uspace_log10dose_nolegend.png",
...
ggsave("BT228_Uspace_prolif_nolegend.png",
       dpi = 900, height = 1.25, width = 1.5)
...
ggsave("BT228_Uspace_drug_noleg.png",
       dpi = 900, height = 1.25, width = 1.5)
...
MrVI_z_space <- read_csv("bt228_mrvi_latent_z_umap_02242024.csv", 
                         col_names = TRUE, 
                         col_select = -1) %>%
  as.data.frame() %>%
...
reducedDims(cds_BT228)[["UMAP_Z"]] <- MrVI_z_space

plot_cells_U(cds_BT228, reduction_method = "UMAP_Z",
             color_cells_by = "log10_dose",
             cell_size = .5, alpha = 1, 
             label_cell_groups = F) +
  # facet_wrap(~log10_dose) +
  ...
  guides(color = "none") +
  viridis::scale_color_viridis(name = "Log10 Dose", discrete = FALSE, option = "magma")
ggsave("BT228_Zspace_log10dose_nolegend.png",
       dpi = 900, height = 1.25, width = 1.5)
...
plot_cells_U(cds_BT228, reduction_method = "UMAP_Z",
  ...
  guides(color = "none") +
  scale_color_manual(values = color_scheme_rand)
ggsave("BT228_Zspace_drug_no_leg.png",
       dpi = 900, height = 1.25, width = 1.5)

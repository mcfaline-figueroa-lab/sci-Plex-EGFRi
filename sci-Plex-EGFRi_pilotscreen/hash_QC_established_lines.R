...
colData_summary <- colData(cds) %>% as.data.frame() %>% group_by(Cell.Line) %>% summarise(count = n(), avg.umi = mean(n.umi))
...
ggplot(data = as.data.frame(colData(cds)) %>% filter(is.na(Cell.Line) == FALSE)) +
  ...
  annotate(geom = "text", label = "Cutoff = 100", x = log10(320), y = 3.4) + 
...
# Creating Knee plot for UMI cutoff by cell line
A172 <- cds[,is.na(colData(cds)$Cell.Line) == FALSE & colData(cds)$Cell.Line == "A172"]
col_A172 <- colData(A172)
rm(A172)
T98G <- cds[,is.na(colData(cds)$Cell.Line) == FALSE & colData(cds)$Cell.Line == "T98G"]
col_T98G <- colData(T98G)
rm(T98G)
U87MG <- cds[,is.na(colData(cds)$Cell.Line) == FALSE & colData(cds)$Cell.Line == "U87MG"]
col_U87MG <- colData(U87MG)
rm(U87MG)
...
ggplot(colData(cds) %>% as.data.frame() %>% arrange(desc(n.umi)) %>% mutate(cell_rank = dplyr::row_number()), aes(x= log10(cell_rank), y = log10(n.umi))) +
  geom_point(size = 0.5, stroke = 0, aes(color = "Total")) +
  geom_point(data = col_A172 %>% as.data.frame() %>% arrange(desc(n.umi)) %>% mutate(cell_rank = dplyr::row_number()),
             aes(x= log10(cell_rank), y = log10(n.umi), color = "A172"), size = 0.5, stroke = 0) +
  geom_point(data = col_T98G %>% as.data.frame() %>% arrange(desc(n.umi)) %>% mutate(cell_rank = dplyr::row_number()),
             aes(x= log10(cell_rank), y = log10(n.umi), color = "T98G"), size = 0.5, stroke = 0) +
  geom_point(data = col_U87MG %>% as.data.frame() %>% arrange(desc(n.umi)) %>% mutate(cell_rank = dplyr::row_number()),
             aes(x= log10(cell_rank), y = log10(n.umi), color = "U87MG"), size = 0.5, stroke = 0) +
  ...
  viridis::scale_color_viridis(name = "Cell Line", 
                              breaks = c("Total","A172", "T98G", "U87MG"),
                              discrete = TRUE,
                              option = "C") +
  xlab("Log10 CellRank") +
  ylab("Log10 UMI") +
...
rm(col_A172, col_T98G, col_U87MG)
...
test <- exprs(cds)
test <- t(test)
Matrix::writeMM(test, "UMI_count_filt.matrix") 
# import this file into scrublet .ipynb to get doublet_scores

scores <- read.table(file = "doublet_scores_EGFRi.txt",
                     header = F)
...
dim(cds)
cds <- cds[, colData(cds)$hash_umis >= 100 &
             colData(cds)$top_to_second_best_ratio >= 3]
dim(cds)
...
# reassignment based on mislabeled hash samplesheet
col_reassign <- colData(cds) %>% as.data.frame() %>%
  mutate(treatment = case_when(
    plate_position == "A03" ~ "Afatinib",
    plate_position == "B03" ~ "Afatinib",
    plate_position == "C03" ~ "Afatinib",
    plate_position == "D03" ~ "Afatinib",
    plate_position == "E03" ~ "Afatinib",
    plate_position == "F03" ~ "Afatinib",
    plate_position == "G03" ~ "Afatinib",
    TRUE ~ treatment
  )) %>%
...
col_reassign <- DataFrame(col_reassign)
colData(cds) <- col_reassign

saveRDS(object = cds, file = "cds_QC_establishedlines.RDS")
